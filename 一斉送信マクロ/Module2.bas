...
Sub AutoCheck_And_SendMail()
    ...
    For i = 4 To lastMailRow
        If wsMail.Cells(i, "A").Value = True Then
            ...
            ' 金額 lookup from LOG（C=本来の金額, D=実際に支給した金額, E=差額）
            ...
            mailBody = Replace(mailBody, "[本来の金額]", honraiAmt)
            mailBody = Replace(mailBody, "[実際に支給した金額]", shikyuAmt)
            mailBody = Replace(mailBody, "[差額]", sagakuAmt)
