...
ErrHandler:
Application.StatusBar = False
MsgBox "一括実行中にエラーが発生しました。" & vbCrLf & _
...
If kihonSheetName <> "" Then
    ...
    Set ws3Kihon = wb3.Worksheets(kihonSheetName)
    On Error GoTo 0
...
If i = 3 Then
    ...
    Debug.Print "166列目のヘッダー: " & ws3.Cells(1, 166).Value
    ...
    For j = 3 To MR0
        ...
        If (j - 3) Mod 8 = 0 Then
            ...
            If Not FCell Is Nothing And Trim(currentEmpNo) <> "" Then
                ...
                ' === 給与明細データの値確認 ===
                Debug.Print "  源泉対象額(166列): " & ws3.Cells(wRow, 166).Value
                If wRowBase > 0 Then Debug.Print "  基本給参照(166列): " & ws3Kihon.Cells(wRowBase, 166).Value
            ...
            If Not wsDell Is Nothing And T_mon2 > 0 And wRow > 0 Then
                ...
                If Not fEmpFS Is Nothing Then
                    ...
                    If baseRowFS + 1 <= wsDell.Rows.Count Then
                        ...
                        If wRowBase > 0 Then
                            wsDell.Cells(baseRowFS + 1, T_mon2).Value = ws3Kihon.Cells(wRowBase, 166).Value '源泉対象額（基本給参照シート優先）
                        Else
                            wsDell.Cells(baseRowFS + 1, T_mon2).Value = ws3.Cells(wRow, 166).Value '源泉対象額
                        End If
        ...
        If (j - 3) Mod 8 = 1 Then
            ...
            If wRowBase > 0 Then
                ws0.Cells(j, T_mon).Value = ws3Kihon.Cells(wRowBase, 166).Value '源泉対象額（基本給参照シート優先）
            ElseIf wRow > 0 Then
                ws0.Cells(j, T_mon).Value = ws3.Cells(wRow, 166).Value '源泉対象額
            End If
